...
Moreover, $b_1(M^d) = d$ if and only if $M^d$ is isometric to the flat torus $\mathbb{T}^d$. 
If additionally $\operatorname{Ric}^g|_{TL\times TL} > 0$ and $d \in \{3,4\}$, then $b_1(M^d) \in \{1, d\}$.
\end{thm}
